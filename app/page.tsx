...
import { Plus, Search, Grid, List, Settings } from 'lucide-react'
...
export default function Home() {
  ...
  const addApp = (newApp: Omit<App, 'id' | 'createdAt'>) => {
    const app: App = { ...newApp, id: Date.now().toString(), createdAt: new Date() }
    setApps([...apps, app])
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="input-field max-w-xs">
              {categories.map(category => (
                <option key={category} value={category}>{category === 'all' ? '所有分类' : category}</option>
              ))}
            </select>
          </div>
          
          <div className="flex justify-between items-center">
            <p className="text-gray-600">共找到 {filteredApps.length} 个应用</p>
        ...
        {filteredApps.length === 0 ? (
          ...
          <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6' : 'space-y-4'}>
            {filteredApps.map(app => (
              <AppCard key={app.id} app={app} viewMode={viewMode} onDelete={deleteApp} onStatusChange={updateAppStatus} />
            ))}
          </div>
        )}
      </main>
